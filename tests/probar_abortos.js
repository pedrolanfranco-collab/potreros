/*
 * Prueba ad-hoc: registro de Abortos en Herramientas (19/9/2026).
 * Mismo patrón que "🌧 Lluvias"/"❄️ Otros eventos" (probar_lluvias.js):
 * evento del establecimiento entero, sin potrero, aplicado en
 * aplicarEventoRemoto ANTES del guard que exige un potrero válido. No
 * toca stock (el feto nunca se contó como animal vivo) -- solo entra al
 * % de abortos junto al de nacidos (ver calcularEstadisticasNacimientos,
...
async function probarArchivo(archivo){
  ...
  chequear('estado.abortos arranca vacío', Array.isArray(win.__est().abortos) && win.__est().abortos.length===0);
  chequear('boton btn-lluvias presente (Abortos vive en el mismo modal)', !!doc.getElementById('btn-lluvias'));
  chequear('selector de motivo presente', !!doc.getElementById('ab-motivo'));
  chequear('boton ab-guardar presente', !!doc.getElementById('ab-guardar'));

  // --- cargar un aborto local ---
  doc.getElementById('btn-lluvias').dispatchEvent(new win.Event('click', { bubbles: true }));
  doc.getElementById('ab-motivo').value = 'no_prenada';
  doc.getElementById('ab-obs').value = 'Potrero 9, vaca de Silvia';
  doc.getElementById('ab-guardar').dispatchEvent(new win.Event('click', { bubbles: true }));
  const abortos = win.__est().abortos;
  chequear('el aborto queda en estado.abortos', abortos.length === 1 && abortos[0].motivo === 'no_prenada' && abortos[0].obs === 'Potrero 9, vaca de Silvia',
    JSON.stringify(abortos));
  chequear('la lista se re-renderiza con la carga',
    /diagnosticada preñada que no lo está/.test(doc.getElementById('abortos-lista').innerHTML));
  ...
  doc.getElementById('ab-fecha').value = '';
  doc.getElementById('ab-guardar').dispatchEvent(new win.Event('click', { bubbles: true }));
  chequear('sin fecha no carga nada', win.__est().abortos.length === antes);
  ...
  win.aplicarEventoRemoto({ tipo:'aborto', potrero: 'esto-no-es-un-potrero', dispositivo:'otro',
    fecha_cliente: '02/09/2026', detalle: { id:'remoto1', motivo:'feto_visto', obs:null, usuario:'Otro' } });
  chequear('un evento de aborto remoto se aplica sin requerir un potrero válido',
    win.__est().abortos.some(ab=>ab.id==='remoto1' && ab.motivo==='feto_visto'));
  ...
  const cantesDeRepetir = win.__est().abortos.length;
  win.aplicarEventoRemoto({ tipo:'aborto', potrero: 'esto-no-es-un-potrero', dispositivo:'otro',
    fecha_cliente: '02/09/2026', detalle: { id:'remoto1', motivo:'no_prenada', obs:'otra cosa', usuario:'Otro' } });
  chequear('no duplica un evento de aborto con el mismo id', win.__est().abortos.length === cantesDeRepetir);
  ...
  doc.getElementById('ab-guardar').dispatchEvent(new win.Event('click', { bubbles: true }));
  await new Promise(r=>setTimeout(r, 30));
  chequear('sin señal el evento de aborto queda en cola', (win.__est().colaSync||[]).length === colaAntes+1);
  Object.defineProperty(win.navigator, 'onLine', { value: true, configurable: true });
  await win.vaciarColaSync();
  chequear('al volver la señal la cola se vacía', (win.__est().colaSync||[]).length === 0);
  chequear('ese evento en cola también llegó al servidor', servidor.filas.filter(f=>f.tipo==='aborto').length >= 2);
}
