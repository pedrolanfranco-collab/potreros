...
async function probarLaVuelta(archivoPc, archivoMovil){
  ...
  for(const archivo of [archivoPc, archivoMovil]){
    ...
    win.document.getElementById('ca-cat').value = 'Vacas';
    win.document.getElementById('ca-dueno').value = win.__config().duenos ? win.__config().duenos[0] : '';
    win.document.getElementById('ca-cant').value = '4';
    win.document.getElementById('ca-fecha').value = win.fechaISOHoy();
    win.document.getElementById('ca-guardar').click();
    chequear(`${archivo}: envío a campo ajeno resta del potrero`, win.__est().potreros[potrero].animales[keyVacasDueno0] === 6, win.__est().potreros[potrero].animales[keyVacasDueno0]);
    chequear(`${archivo}: envío a campo ajeno suma al balde estado.campoAjeno`, win.__est().campoAjeno.animales[keyVacasDueno0] === 4);
    ...
    win.document.getElementById('ca-cant').value = '4';
    win.document.getElementById('ca-guardar').click();
    chequear(`${archivo}: retorno de campo ajeno devuelve al potrero`, win.__est().potreros[potrero].animales[keyVacasDueno0] === 10);
    chequear(`${archivo}: retorno de campo ajeno resta del balde`, win.__est().campoAjeno.animales[keyVacasDueno0] === 0);

    // ---- 5) Borrar un envío revierte bien ----
    setValor(win, 'ca-tipo', 'envio');
    win.document.getElementById('ca-potrero').value = potrero;
    win.document.getElementById('ca-cant').value = '3';
    win.document.getElementById('ca-guardar').click();
    ...
    chequear(`${archivo}: borrar el envío devuelve el stock al potrero`, win.__est().potreros[potrero].animales[keyVacasDueno0] === 10);
    chequear(`${archivo}: borrar el envío saca del balde de campo ajeno`, win.__est().campoAjeno.animales[keyVacasDueno0] === 0);
    chequear(`${archivo}: el envío queda marcado ELIMINADO`, win.__est().potreros[potrero].historial[0].eliminado === true);

    // ---- 6) Sincronización de envío/retorno a otro dispositivo ----
    ...
    otro2.win.aplicarEventoRemoto({
      ...
      detalle: {categoria:'Vacas', dueno: win.__config().duenos ? win.__config().duenos[0] : null, cantidad: 5}
    }, new Set());
